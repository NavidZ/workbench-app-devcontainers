...
func buildContext(dc *DataCollection, profile *DataProfile) string {
	...
	b.WriteString("\n== DATA PROFILE ==\n")
	b.WriteString("Each asset notes the DC Folder it lives under (a path like `Version 1/raw`). ")
	b.WriteString("Use this organization when it is meaningful — e.g. describe how the data is grouped, ")
	b.WriteString("or reflect the folder structure in the Data model or Data snapshot — but do not over-emphasize it.\n")
