...
type TableProfile struct {
	...
	FolderPath  string         `json:"folderPath,omitempty"` // DC folder chain the dataset lives under
...
type BucketProfile struct {
	...
	FolderPath  string        `json:"folderPath,omitempty"` // DC folder chain the bucket lives under
...
func buildDataProfile(ctx context.Context, resources []Resource, prog progressFunc) *DataProfile {
	...
	for _, r := range resources {
		...
		case "BIG_QUERY_DATASET":
			...
			for i := range tables {
				tables[i].FolderPath = r.FolderPath
			}
