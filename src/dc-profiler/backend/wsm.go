...
type Resource struct {
	...
	// FolderID is the WSM folder the resource lives in (empty = workspace root).
	FolderID string `json:"folderId,omitempty"`
	// VersionID is the top-level folder (DC version) the resource belongs to,
	// or noVersionID for resources at the workspace root. Populated by
	// annotateResourceVersions, not by WSM.
	VersionID string `json:"versionId,omitempty"`
	// FolderPath is the human-readable folder chain the resource lives under
	// (e.g. "Version 1/raw/2023"), populated by annotateResourceVersions.
	FolderPath string `json:"folderPath,omitempty"`
}

// Folder is a trimmed WSM folder. DC versions are implemented as top-level
...
func listResources(id string) ([]Resource, error) {
	...
	for _, r := range raw.Resources {
		res := Resource{Name: r.Metadata.Name, ResourceType: r.Metadata.ResourceType, FolderID: r.Metadata.FolderID}
...
// listFolders returns the workspace's folders. DC versions are the top-level
// ones (ParentFolderID == "").
func listFolders(id string) ([]Folder, error) {
	var raw struct {
		Folders []Folder `json:"folders"`
	}
	path := "/api/workspaces/v1/" + id + "/folders"
	if err := wsmRequest(http.MethodGet, path, nil, &raw); err != nil {
		return nil, err
	}
	return raw.Folders, nil
}
