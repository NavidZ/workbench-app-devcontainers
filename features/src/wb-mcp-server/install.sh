...
# Write MCP config to ~/.mcp.json (project-scoped, read by Claude Code v2.1+).
# claude mcp add can be unreliable across versions, so write the file directly.
...
# Also try claude mcp add as a fallback
